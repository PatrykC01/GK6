...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {    
		...
		int wierzcholki = 7;
		double przyrost = 2 * Math.PI / wierzcholki;
		double a = 0;
		int radius = 6;
		float x = 0;
		float z = 0;
		...
		for(int i = 1; i <= wierzcholki; i++)
		{
			...
			z = (float) (radius * Math.sin(a));
			
			a = i * przyrost;
			
			float x0 = (float) (radius * Math.cos(a));
			float z0 = (float) (radius * Math.sin(a));
	...
	public void init(GLAutoDrawable drawable) {
		...
		float[] s1 = {0.4f, 0.4f, 0.4f, 1};
		float[] s2 = {0, 0, 0.8f, 1};
		float[] s3 = {0.3f, 0.15f, 0.15f, 1};
		
		gl2.glLightfv(gl2.GL_LIGHT1,gl2.GL_DIFFUSE, s1, 0);
		gl2.glLightfv(gl2.GL_LIGHT1,gl2.GL_SPECULAR, s2, 0);
		gl2.glLightfv(gl2.GL_LIGHT1,gl2.GL_AMBIENT, s3, 0);
	
		float[] pozycja = {1, 0, 0, 0};
		
		gl2.glLightfv(gl2.GL_LIGHT1, GLLightingFunc.GL_POSITION, pozycja, 0);
